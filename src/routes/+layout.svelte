...
{#if mounted}
<div bind:this={layoutContainer} class="min-h-screen md:h-screen md:overflow-hidden flex flex-col md:flex-row islamic-bg soft-gradient {$isPremium ? 'premium-theme' : ''}">
  
  <!-- DESKTOP SIDEBAR -->
  <aside class="desktop-sidebar hidden md:flex flex-col glass border-r border-white/5 p-6 h-screen sticky top-0 shrink-0 z-20 justify-between self-start transition-all duration-300 {sidebarCollapsed ? 'w-20' : 'w-64 lg:w-72'} {$isPremium ? 'premium-border' : ''}">
    <div class="flex flex-col gap-8">
      <!-- App Logo & Brand & Toggle -->
      <div class="flex items-center justify-between px-1">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-xl flex items-center justify-center shadow-lg transition-all duration-300 shrink-0
            {$isPremium 
          ...
          {#if !sidebarCollapsed}
            <div class="transition-all duration-300">
              <h1 class="font-bold text-sm lg:text-base tracking-wide flex items-center gap-1
                {$isPremium ? 'premium-gold-text' : 'text-emerald-500 dark:text-emerald-400'}">
                CREATIVE QUR'AN
              </h1>
              <span class="text-[10px] font-extrabold tracking-widest uppercase block -mt-0.5
      ...
      <nav class="flex flex-col gap-2">
        {#each menuItems as item (item.path)}
          ...
          <a 
            ...
            class="flex items-center rounded-2xl text-sm font-semibold transition-all duration-300 group relative overflow-hidden
              {sidebarCollapsed ? 'justify-center p-3.5' : 'gap-3.5 px-4 py-3.5'}
              {isActive(item.path) 
                ? ($isPremium 
          ...
          >
            ...
            <svelte:component this={Icon} class="w-5 h-5 transition-transform duration-300 group-hover:scale-110 shrink-0
              {isActive(item.path) 
        ...
        {#if showPremiumBtn}
          {#if !$isPremium}
            ...
            >
              <Crown class="w-5 h-5 text-amber-400 fill-amber-400 animate-pulse-slow shrink-0" />
              {#if !sidebarCollapsed}
          ...
          {:else}
            ...
            >
              <BookMarked class="w-5 h-5 text-amber-400 fill-amber-400 animate-pulse-slow shrink-0" />
              {#if !sidebarCollapsed}
    ...
    <div class="flex flex-col gap-4">
      {#if $lastRead}
        <a 
          ...
          class="flex items-center rounded-2xl bg-white/5 border border-white/5 hover:border-emerald-500/20 group cursor-pointer {$isPremium ? 'premium-border' : ''}
            {sidebarCollapsed ? 'justify-center p-3.5' : 'gap-3 p-3.5'}"
          title={sidebarCollapsed ? `Terakhir Baca: ${$lastRead.surahName} (Ayat ${$lastRead.ayahNumber})` : ''}
        >
      ...
      {:else}
        <div class="text-center pt-2 border-t border-white/5 text-[9px] text-zinc-600 font-medium">
          v1.0
        </div>
      {/if}
    </div>
